    ...
    if _pool is None:
        with _pool_lock:
            if _pool is None:
                ...
                _pool = ThreadedConnectionPool(minconn=2, maxconn=100, **params)
    return _pool
...
def export_tables_to_csv(output_dir: str) -> None:
    ...
    tables_order = {
        ...
        "raw_metrics": "timestamp",
        "results_log": "id",
        "scenario_summary": "aggregated_at",
    }

    for table, order_col in tables_order.items():
        with get_connection() as conn:
            with conn.cursor() as cur:
                output_path = os.path.join(output_dir, f"{table}.csv")
                with open(output_path, "w") as f:
                    cur.copy_expert(
                        f"COPY (SELECT * FROM load_test.{table} ORDER BY {order_col}) TO STDOUT WITH CSV HEADER",
                        f
                    )
